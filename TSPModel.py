...
class TSPModel(nn.Module):
    ...
    def __init__(self, **model_params):
        ...
        decoder_variant = self.model_params.get('decoder_variant', 'pomo_light')
        if decoder_variant == 'pomo_light':
            self.decoder = POMO_Light_Decoder(**model_params)
        elif decoder_variant == 'po_heavy':
            self.decoder = PO_Heavy_Decoder(**model_params)
        else:
...
class PO_Heavy_Decoder(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
...
class POMO_Light_Decoder(nn.Module):
    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        embedding_dim = self.model_params['embedding_dim']
    ...
    def set_q1(self, encoded_q1, expand=None):
        batch_size = encoded_q1.size(0)
        embedding_dim = encoded_q1.size(2)
        head_num = self.model_params['head_num']

        if expand is not None:
            encoded_q1 = encoded_q1.unsqueeze(2).expand(-1, -1, expand, -1).contiguous()
            encoded_q1 = encoded_q1.view(batch_size, -1, embedding_dim)
