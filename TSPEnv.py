...
class TSPEnv:
    def __init__(self, **env_params):
        ...
        self.device = env_params.get('device')
        ...
        # Optional TSPLIB/lib evaluation fields.
        self.edge_weight_type = None
        self.original_node_xy_lib = None

    def _move_to_runtime_device(self, tensor):
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1):
        ...
        if problems is not None:
            ...
            self.problems = self._move_to_runtime_device(problems)
        elif self.validation_set_path is not None:
            self.problems = self._move_to_runtime_device(
                self.loaded_problems[self.batch_count*self.batch_size:(self.batch_count+1)*self.batch_size]
            )
            self.batch_count += 1
        else:
            self.problems = self._move_to_runtime_device(get_random_problems(batch_size, self.problem_size))
    ...
    def step(self, selected=None, lib_mode: bool = False):
    ...
    def _get_travel_distance(self, lib_mode: bool = False):
        ...
        if lib_mode:
            edge_weight_type = self.edge_weight_type or 'EUC_2D'
