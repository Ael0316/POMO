...
class TSPTester_LIB:
        ...
        for scale_range in scale_range_all:
            ...
            for root, _, files in os.walk(filename):
                for file in files:
                    ...
                    self.logger.info("===============================================================")
                    self.logger.info(
                        "Instance name: {}, problem_size: {}, EDGE_WEIGHT_TYPE: {}".format(
                            name,
                            dimension,
                            ew_type,
                        )
                    ...
                    try:
                        ...
                        self.logger.exception(f"Failed on instance {name}: {exc}")
                        continue
    ...
    def _test_one_instance(
        self,
        nodes_xy_normalized: torch.Tensor,
        ...
        env = Env(problem_size=problem_size, pomo_size=problem_size, device=self.device)
